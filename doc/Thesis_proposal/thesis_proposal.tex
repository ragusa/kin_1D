...
Transient modeling of nuclear reactors has been a chronically foreboding task due to its computationally expensive nature.  However, recent developments in nuclear testing, including the revitalization of the Transient Reactor Testing (TREAT) Facility at Idaho National laboratory (INL), have brought significant attention to the development of transient reactor modeling.  Transient computational methods currently in \tcr{laboratory frameworks--people may not understand what you mean here} have proven to be overbearingly \tcr{slow-CPU-expensive} for complex, real-world scenarios.  Therefore, more efficient and faster methods for computational \tcr{frameworks} are highly desired.  The Department of Energy (DOE) has especially sought to develop in the MOOSE framework.  \\
...
To derive the amplitude equation, the shape/precursors equations are weighted by a space-dependent function and integrated over the phase-space. The weight function is typically the adjoint flux $\phi^{*g}$, which can be proven to minimize truncation error \cite{duderstadt1976nuclear}.  \tcr{Check Cristian Rabitti's PhD work. There is more to it. He used a new adjoint at each time step I believe. It is interesting to read}
The final expressions are given below:
\begin{subequations}
\be
...
To investigate IQS's performance with step-doubling time adaptation, the adaptation will be applied to brute force method, traditional IQS, and IQS P-C.  Each of these methods will be applied to several diffusion problems; the number of time steps taken and the resulting error will be used to compare the methods.
...
\item Implement step doubling time adaptation with IQS and analyze its performance compared to brute-force.
...
\tcr{for a paper, can we think of testing Embeded RK instead of/in addition to step-doubling?}
