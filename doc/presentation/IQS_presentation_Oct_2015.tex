...
\bi
\item action system
\item postprocessors to compute $\rho$, $\bar{\beta}$, $\bar{\lambda}$
\item IQS userobject (for PRKE solve using updated reactivity coefficients, $\rho$, $\bar{\beta}$,
$\bar{\lambda}$)
...
\begin{block}{Add a couple of MOOSE graphs from our physor paper}
\end{block}
