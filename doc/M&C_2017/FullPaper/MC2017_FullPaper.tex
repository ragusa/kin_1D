...
The purpose of this paper is to present a new technique in dealing with multi-physics feedback in transient reactor simulations, specifically utilizing the improved quasi-static method (IQS).  the technique involves the addition of an intermediate time scale to evaluate temperature and PRKE parameters.  The theory is that temperature and the parameters are more quickly varying in time than the shape, thus further reducing the number of multi-group diffusion evaluations and execution time for a given accuracy. The technique was implemented into the Rattlesnake/MOOSE framework at Idaho National Laboratory (INL) and tested with the LRA benchmark and the Transient-15 TREAT problem.  The results proved that the addition of this time scale was effective in reducing execution time for these problems while maintaining accuracy. 
}\par}
\vspace*{14pt}
\end{strip}
...
To test the multiple time scale implementation of IQS, the LRA benchmark was chosen for reference and a TREAT example was employed. The performance of the implementation is evaluated comparing errors in power at peak points with brute force and IQS without quasi-static temperature treatment.  Execution times were also compared at given error values to judge the computational efficiency of adding more temperature evaluations.  These examples were executed using the MOOSE/Rattlesnake framework at INL, which uses a PJFNK (preconditioned Jacobian-free Newton Krylov) method to evaluate the nonlinear problems.  Each time step evaluation consists of three iteration loops: GMRES iterations for the linear system, Newton iterations for the temperature-flux nonlinearity, and Picard iterations for the shape-amplitude nonlinearity.  Another judge of computational expense is the total number of linear iterations during the execution, these are shown specifically for the brute force LRA simulations and all the simulations for the TREAT example.
...
In conclusion, the multi-physics treatment with IQS presented in this paper is worth further investigation, optimization, and implementation into other transient reactor problems with more feedback mechanisms.   
