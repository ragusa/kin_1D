...
\chapter{\uppercase{Improved Quasi-Static Method}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Theory}
\label{sect:theory}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\frac{ \sum_{g=1}^G\left(\phi^{*g},\frac{\chi_p^g}{\keff}\sum_{g'=1}^G \nu^{g'}_p \Sigma_f^{g'}\varphi^{g'} + \sum_{g'\neq g}^G\Sigma_s^{g'\to g} \varphi^{g'} +\left( \div D^g \grad  - \Sigma_r^g \right)\varphi^g\right)}
...
\caption{IQS method visualization \cite{PrincePHYSOR2016}}
\label{fig:iqsviz}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Predictor-Corrector IQS (IQS P-C)}
\label{sect:pc}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\be
T^{n+1} = T^n + \frac{\kappa_f}{\rho c_p} \sum_{g=1}^G\left(a_2(\Sigma_f^g\varphi^g)^{n+1} + a_1(\Sigma_f^g\varphi^g)^n\right)
\label{eq:temp_an}
\ee
...
\draw (6,1) edge[out=45,in=-45,->,thick] node[below,sloped] {$T$} (6,2);
\draw (6,0) edge[out=30,in=0,->,thick] node[below,sloped] {$p, \frac{dp}{dt}$} (6,2);
...
\node[reddiamond] (check1) at (0,-17.5) {Check for \\ convergence ($p,T$)};
\node[reddiamond] (check2) at (7,-12) {If $t=t_{n+1}$};
\node[greenblock] (p7) at (8,-6) {Shape Solve};
\node[reddiamond] (check3) at (8,-2.5) {Check for \\ convergence ($\varphi,K_{n+1}$)};
%\node [above =0.1mm of p1] {\large{IQS Solve:}};
%\node [above =0.1mm of p5] {\large{Multi-physics:}};
...
\draw[->,ultra thick](p2.south) -- node[right] {$t=t_{n}+\Delta t^T$}(p3.north);
\draw[->,ultra thick](p3.south) -- (p4.north);
...
\draw[->,ultra thick](check2.west) -- node[above,sloped] {\large{no}\small \qq $t=t+\Delta t^T$} (4,-5) -- (p3.east);
...
\be
\frac{C^{n+1}_i-C^n_i}{\Delta t}=(1-\theta)\beta_i S_f^np^n-(1-\theta)\lambda_i C^n_i + \theta\beta_i S_f^{n+1}p^{n+1}-\theta\lambda_i C^{n+1}_i
\ee
...
\be
C^{n+1}_i = \frac{1-(1-\theta)\Delta t\lambda_i}{1+\theta\Delta t\lambda_i}C^n_i + \frac{(1-\theta)\Delta t \beta_i}{1+\theta\Delta t\lambda_i}S_f^n p^n +  \frac{\theta\Delta t \beta_i}{1+\theta\Delta t\lambda_i}S_f^{n+1} p^{n+1}
\label{eq:dnp_theta}
\ee
Reporting this value of $C^{n+1}_i$, one can solve for the shape $\varphi^{n+1,g}$ as a function of $\varphi^{n,g}$ and $C^n_i$ (and $p^n$, $p^{n+1}$, $dp/dt|_n$ and  $dp/dt|_{n+1}$). Once $\varphi^{n+1,g}$ has been determined, $C^{n+1}_i$ is updated.  Applying this technique to implicit discretization is done by changing the definition the fission source ($S_f^n=\sum_{g=1}^G(\nu\Sigma_f^g\phi^g)^n$) and eliminating all $p$ terms.
...
\be
C^{n+1}_i =  C^n_i e^{-\lambda_i (t_{n+1} - t_n) }  + \int_{t_n}^{t_{n+1}} \beta_i(t') S_f(t') p(t')e^{-\lambda_i (t_{n+1}-t')}dt'
\label{eq:prec_an}
\ee
...
\be
C^{n+1}_i = C^n_i e^{-\lambda_i \Delta t} + \left(\hat{a}_{2,i} S_f^{n+1}+\hat{a}_{1,i} S_f^n\right)\beta_i
\label{eq:dnp_an}
\ee
With integration coefficients defined as:
\begin{align}
&\hat{a}_{1,i}= \int_{t_n}^{t_{n+1}}\frac{t_{n+1}-t'}{\Delta t}p(t')e^{-\lambda_i(t_{n+1}-t')}dt' \\
&\hat{a}_{2,i} = \int_{t_n}^{t_{n+1}}\frac{t'-t_n}{\Delta t}p(t')e^{-\lambda_i(t_{n+1}-t')}dt'
\end{align}
