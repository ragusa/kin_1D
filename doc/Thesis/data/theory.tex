...
\tcr{I vaguely remind something about $\bar{\lambda}$ not being so easy to obtain/derive/or conceptualize. What was that story?}
...
Where $n$ is the time step, $k$ is the iteration number, and $\epsilon_{\varphi}$ is the numerical criteria provided by a user.  There is no guarantee that the shape converges, so max number of iterations is typically enforced. \tcr{rephrase last bit}
...
\ee
\tcr{here, and before, $n$ seems to indicate the current time at which the solution is sought. I believe that later, the unknowns
will be described as being at $n+1$. this is a slight inconsistency.}
...
\tcr{The $k$ subscript is confusing. It reads like $\alpha_{jk}$ is a matrix entry but the rest of the equation 2.35 is independent of $k$. Something is fishy.}
...
IQS will be made available \tcr{this is a copy paste from an old report but it is no longer fully accurate, right?}
to any action system capable of transient simulation, which includes:
\begin{itemize}
\item CFEM-Diffusion
\item DFEM-Diffusion
\item SAAF-CFEM-SN
