...
\ee
\be
\Delta t_{new} = \Delta t \left(\frac{e_{tol}}{e_n}\right)^{\frac{1}{1+\mu}}
\label{eq:dt2}
\ee
\tcr{change subscripts}
...
\subsection{Input}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\tcr{make sure boxes are nice int he final version}
...
\caption{Error convergence comparison of 1D heterogeneous example}
\label{fig:conv}
\end{figure}
...
\tcr{not an easy problem because shape changes in time a lot. 1 figure, shape at 3 times: t1=0, cosine, t2=max deform, t3=end simulation}
