...
\ee 
\end{subequations}
The following inner product definition has been used: $\left(\phi^{*g},f\right):=\int_D\phi^{*g}(\vec{r})f(\vec{r})d^3r$.  Additionally, in order to impose uniqueness on the factorization and to derive the PRKE the following normalization condition is imposed: $\sum_{g=1}^G\left(\phi^{*g},\frac{1}{v^g}\varphi^g\right) = \textit{constant}$.  This condition is later used  as the convergence criteria when solving the nonlinear system of equations.
...
\centering
\begin{tikzpicture}[
redbox/.style = {rectangle, rounded corners, draw, align=center, top color=white, bottom color=red!20, ultra thick, minimum size=20mm}, 
bluebox/.style = {rectangle, rounded corners, draw, align=center, top color=white, bottom color=blue!20, ultra thick, minimum size=20mm},
greenbox/.style = {rectangle, rounded corners, draw, align=center, top color=white, bottom color=green!20, ultra thick, minimum size=20mm},
every node/.style       = {font=\large}
]
\node[redbox](iqs) at (0,0) {IQS};
\node[bluebox](shape) at (6,2) {Shape};
\node[greenbox](prke) at (11,2) {PRKE};
\node[bluebox](mp) at (6,-2) {Multi-Physics};

\draw[<->,ultra thick,](iqs.north east) -- node[above,sloped] {Picard}(shape.west);
%\draw[<->,ultra thick](iqs.east) -- node[above] {Picard}(prke.west);
\draw[<->,ultra thick](iqs.south east) -- node[below,sloped] {Picard} (mp.west);

\draw[->,ultra thick](prke.south) to [bend left=60] node[above,sloped,midway] {$p,\frac{dp}{dt}$}(shape.south);
\draw[->,ultra thick](shape.east) -- (prke.west) node[above,sloped,midway] {$\rho,\bar{\beta},\bar{\lambda},\Lambda$};
