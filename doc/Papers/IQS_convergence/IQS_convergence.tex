...
\ee
to be constant (hence $K(t)=K_0$).  
After some manipulation, the point reactor kinetics equations (PRKE) for the amplitude solution are finally obtained:
\begin{subequations}
\be
...
%---------------------------------------------------------------------------------------%
\comm{make sure the intro has a decent overview -ie no equation, just words- on the IQS-PC. Dulla and Ravetto will likely review this paper}
...
at the beginning and end times of the macro time step are used (and interpolated in between).
...
The advantage to the predictor-corrector method is there is no iteration necessary for this method and, in turn, 
is much simpler and can be faster than the standard IQS.  Ikeda et al. in \cite{Ikeda_2001} and Goluoglu et al. 
in \cite{Goluoglu_2001} both use IQS-PC for complex, three-dimensional problems.  Their results prove IQS P-C to be impressively effective, despite the de-coupling of the system.  Dulla et al. in \cite{Dulla2008} also describes an in depth comparison of IQS P-C with traditional IQS.
