...
\begin{table}[htbp!]
\centering
\caption{Operator definition of multi-group neutron conservation equations\tcr{why do you have transfers in $\chi_p$ for $P$?}}
\label{tab:op_def}
\def\arraystretch{2}
...
\hline
$P_p^{g'\to g}$ & Prompt fission & $\frac{1}{4\pi}\chi_p^{g'\rightarrow g}\frac{\nu_p\Sigma_f^{g'}}{\keff}\int_{4\pi}d\Omega '$ & $\chi_p^{g'\rightarrow g}\frac{\nu_p\Sigma_f^{g'}}{\keff}$ \\
\hline
...
\end{tabular}
\end{table}
\tcr{in transport, shouldn't $P^g_{d,i}$ also have a $1/4\pi$?}
%
\noindent
...
%$L^g = -\div D^g \grad + \Sigma_r^g$, with  $\Sigma_t^g$ and $\Sigma_r^g$ the total and removal macroscopic cross sections. 
...
\end{itemize}
\tcr{Why don't you have absolute values in the last 3 criteria?}
...
This idea stems from the fact that in an adiabatic temperature model, the temperature's spatial variation follows the neutron flux shape, while the temperature amplitude follows the flux amplitude. Because of the heat capacity of the fuel, the temperature amplitude is more slowly varying in time than the heat \tcr{?inflow?} from the flux amplitude, while still considerably faster than the time \tcr{?variance?} of the flux shape. This logic is empirically defended in the results of Section 5.
...
all three variables. \fig{fig:proc} shows the solution workflow implemented to execute this process. 
$N_T$ is the number of temperature updates per macro step.
%The time increment of $\Delta t/3$ for the temperature solve is arbitrary and is meant only to match \fig{fig:time} where three temperature updates have been used as illustration.
...
To investigate IQS's performance with step-doubling time adaptation, the adaptation will be applied \tcr{?to direct temporal discretization?} of the flux equation (no IQS), the standard IQS method, and the \iqspc version. Each of these methods will be applied to several diffusion problems; the number of time steps taken and the resulting error will be used to compare the methods.
...
\begin{table}[!htbp]
\begin{center}
\caption{1-D slab absorption cross sections at times of interest}
\label{tab:1Dslope}
\resizebox{\linewidth}{!}{
...
%This illustrates that IQS can yield a much more accurate solution, even at a significantly larger time step than the implicit flux discretization.
...
This observation implies that the time adaptive options must be chosen differently when applied to a solver for shape versus a solver for flux.
...
\caption{Difference in normalized power from reference solution for each fissile block of the LRA geometry}
\label{fig:lra_block}
\end{figure}
...
\caption{Transient-15 Error and Run-time Results}
\label{tab:tran15}
\end{table}
